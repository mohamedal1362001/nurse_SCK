...
import random
class Solution:
    def __init__(self,p=Populations,number_nurses=0,days=0,generations=0,select_rand=0,select_best=0,nmutation=0):
    ...
    def evolve(self,mutate, select_random, select_best, pop=[], fit=[]):
        # select best
        select_best_length = int(len(pop) * (select_best/100))
        parent = pop[:select_best_length]
        parents_fitness = fit[:select_best_length]
        # random select
        for random_child in pop[select_best_length:]:
            if select_random > random.randint(1,100):#select_random > random.random()
        ...
        if len(parent) <= 1:
            ...
            if mutate > random.randint(1,100):# mutate > random.random()
    ...
    def find_best_solution(self):
        ...
        for t in range(0, self.generations - 1):
            if len(pop) <= 1:
                ...
                parents = self.evolve(self.nmutation, self.select_rand, self.select_best, pop, fitness)#parents = self.evolve(0.6, 0.3, 0.2, pop, fitness)
                if len(parents) <= 1:
    ...
    def err_p(self):
        tap = self.get_solution()
        pop = []
        fit = []
        pop.append(tap)
        fit = self.allFitneess(pop)
        return fit[0]
    ...
    def displaytTable(self):
        ...
        data = "wrong -->" + str(self.best_fitness[0]) + "\n"
